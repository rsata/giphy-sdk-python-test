...
pip install git+ssh://git@github.com/rsata/giphy-sdk-python-test.git
```
...
to add `GIPHY_API_KEY="My API Key"` to your `.env` file
...
When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `giphy.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `giphy.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `giphy.APIError`.
...
except giphy.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except giphy.APIStatusError as e:
